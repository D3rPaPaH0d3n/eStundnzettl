...
function readLSInt(key: string, fallback = 0): number {
...
export function useAutoBackup({ entries, userData, isEnabled }: UseAutoBackupProps) {
  const latestDataRef = useRef<LatestData>({ entries, userData });
  const lastHash = useRef<string>("");
  const debounceTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const isUploading = useRef<boolean>(false);
  const [backupFailCount, setBackupFailCount] = useState<number>(
    () => readLSInt(STORAGE_KEYS.BACKUP_FAIL_COUNT)
  );

  // SQLite-Nachladen (einmalig, async)
  const sqliteInitDone = useRef<boolean>(false);
  useEffect(() => {
  ...
  const createHash = (data: LatestData): string => {
  ...
  const clearNextcloudErrorState = async (): Promise<void> => {
  ...
  const registerNextcloudFailure = async (error: unknown): Promise<void> => {
    const current = readLSInt(STORAGE_KEYS.NEXTCLOUD_BACKUP_FAIL_COUNT);
    const newCount = current + 1;
    const message = String(error instanceof Error ? error.message : error || "Nextcloud-Backup fehlgeschlagen");
  ...
  const performBackup = async (source: "Auto-Save" | "Background"): Promise<void> => {
    ...
    try {
      ...
      if (cloudActive) {
        try {
          ...
          if (authResponse) {
            await uploadOrUpdateFile(authResponse.accessToken, BACKUP_CONFIG.FILENAME, JSON.stringify(payload));
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      listenerHandle.current = await App.addListener('appStateChange', ({ isActive }: { isActive: boolean }) => {
