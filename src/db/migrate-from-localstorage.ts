/**
 * migrate-from-localstorage.ts — Einmalige Migration bestehender Daten
 *
 * Welle 1: Entries
...
import { bulkInsertEntries, EntryInput } from "./repositories/entriesRepo";
import { bulkWriteSettings } from "./repositories/settingsRepo";
import { bulkReplaceWorkCodes, WorkCodeInput } from "./repositories/workCodesRepo";
import { bulkReplaceAttachments, bulkReplaceLabelSuggestions, AttachmentInput } from "./repositories/attachmentsRepo";
...
export interface AttachmentsMigrationResult extends MigrationResult {
  labels: number;
}
 ...
 *
 * @returns {Promise<AttachmentsMigrationResult>}
 */
export async function migrateAttachmentsToSQLite(): Promise<AttachmentsMigrationResult> {
...
export interface AllMigrationResults {
  entries: MigrationResult | MigrationErrorResult;
  settings: MigrationResult | MigrationErrorResult;
  workCodes: MigrationResult | MigrationErrorResult;
  attachments: AttachmentsMigrationResult | MigrationErrorResult;
  backupMeta: MigrationResult | MigrationErrorResult;
}
 ...
 *
 * @returns {Promise<AllMigrationResults>}
 */
export async function migrateAllToSQLite(): Promise<AllMigrationResults> {
  const results: AllMigrationResults = { 
    entries: { migrated: 0, skipped: true, error: 'not started' }, 
    settings: { migrated: 0, skipped: true, error: 'not started' }, 
    workCodes: { migrated: 0, skipped: true, error: 'not started' }, 
    attachments: { migrated: 0, labels: 0, skipped: true, error: 'not started' }, 
    backupMeta: { migrated: 0, skipped: true, error: 'not started' } 
  };
