/**
 * pdfArchiveTargets.ts — Ziel-Adapter fuer das automatische PDF-Archiv.
 *
...
import * as nextcloudClient from "./nextcloudClient";
const { uploadBinaryToPath } = nextcloudClient as any;
 ...
 */
export async function writeLocalArchive(filename: string, base64: string, blob?: Blob): Promise<ArchiveResult> {
  if (!Capacitor.isNativePlatform()) {
    // Web: schlichter Download als Fallback
    if (!blob) return { ok: false, error: "Kein Blob fuer Web-Download", target: "local-web" };
 ...
 */
export async function uploadGDriveArchive(filename: string, base64: string, blob?: Blob): Promise<ArchiveResult> {
  if (!base64) {
