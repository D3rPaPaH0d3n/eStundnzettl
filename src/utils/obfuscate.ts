 ...
 */
export function _resetObfuscateCacheForTests(): void {
  _cachedKeyPromise = null;
}
 ...
 */
export async function obfuscate(val: string | null | undefined): Promise<string> {
  ...
  try {
    ...
    const ct = new Uint8Array(
      await crypto.subtle.encrypt(
        { name: "AES-GCM", iv },
        key,
        utf8Encode(str).buffer as ArrayBuffer
      )
    );
 ...
 */
export async function deobfuscate(val: string | null | undefined): Promise<string> {
  if (!val) return "";
  const str = String(val);
  ...
  if (str.startsWith(ENC_PREFIX)) {
    try {
      ...
      if (sep < 0) return "";
      const iv = base64ToBytes(body.slice(0, sep)) as Uint8Array;
      const ct = base64ToBytes(body.slice(sep + 1)) as Uint8Array;
      const crypto = getCrypto();
      const key = await getMasterKey();
 ...
 */
export function deobfuscateLegacySync(val: string | null | undefined): string {
  if (!val) return "";
  const str = String(val);
