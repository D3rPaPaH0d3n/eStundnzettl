...
import { Entry, UserData, TimeEntry } from "../../types";
...
interface UseOnboardingActionsProps {
  ...
  importEntries: (entries: Entry[] | TimeEntry[]) => void;
...
interface LoadFromLocalStorageProps {
  setUserData: (userData: UserData) => void;
  setAutoBackup: (enabled: boolean) => void;
  importEntries: (entries: Entry[]) => void;
}

function loadFromLocalStorage({ setUserData, setAutoBackup, importEntries }: LoadFromLocalStorageProps) {
