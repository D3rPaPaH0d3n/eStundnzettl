...
import { usePeriodStats } from "./hooks/usePeriodStats"; // <--- NEU
...
export default function App() {
  ...
  useEffect(() => {
    if (autoCheckoutData) {
      ...
      setIsLiveEntry(true); 
      setView("add");
      toast("⚠️ Automatisch ausgestempelt! Bitte prüfen.", { duration: 6000, icon: "🌙" });
      clearAutoCheckout();
    }
  }, [autoCheckoutData]);
  ...
  // --- NEU: EINSATZ DES HOOKS ---
  // Wir berechnen die Stats für den AKTUELLEN Monat
  ...
  // Der Hook übernimmt jetzt die komplette Arbeit (Summen, Sollzeit, MA/ÜS)
  const stats = usePeriodStats(entriesWithHolidays, userData, periodStart, periodEnd);
  // --------------------------------

  // Kleine Wrapper für die UI (damit wir nicht alles umbenennen müssen, wo stats benutzt wird)
  // Dashboard erwartet: stats (object), overtime (number), progressPercent (number)
  ...
  const handleStartLive = () => { Haptics.impact({ style: ImpactStyle.Heavy }); startTimer(); toast.success("⏱️ Stempeluhr gestartet!"); };
  ...
  const startNewEntry = () => { setEditingEntry(null); setEntryType("work"); setFormDate(new Date().toISOString().split("T")[0]); setStartTime("06:00"); setEndTime("16:30"); setPauseDuration(30); setProject(""); const lastCode = localStorage.getItem("kogler_last_code"); setCode(lastCode ? Number(lastCode) : WORK_CODES[0].id); setIsLiveEntry(false); setView("add"); };
  const startEdit = (entry) => { setEditingEntry(entry); const isDrive = entry.type === "work" && entry.code === 19; setEntryType(isDrive ? "drive" : entry.type); setFormDate(entry.date); if (entry.type === "work") { setStartTime(entry.start || "06:00"); setEndTime(entry.end || "16:30"); setPauseDuration(isDrive ? 0 : entry.pause ?? 0); setCode(entry.code ?? WORK_CODES[0].id); setProject(entry.project || ""); } else { setPauseDuration(0); setProject(""); } setIsLiveEntry(false); setView("add"); };
  ...
  const handleSaveEntry = (e) => {
    ...
    if (entryType === "work" || isDrive) {
        ...
        if (hasOverlap) { Haptics.impact({ style: ImpactStyle.Heavy }); toast.error("⚠️ Zeitüberschneidung!", { duration: 4000, icon: '⛔' }); return; }
    ...
    const newEntry = { id: editingEntry ? editingEntry.id : Date.now(), type: storedType, date: formDate, start: storedType === "work" ? startTime : null, end: storedType === "work" ? endTime : null, pause: usedPause, project: storedType === "work" ? project : label, code: storedType === "work" ? usedCode : null, netDuration: net };
    if (editingEntry) updateEntry(newEntry); else addEntry(newEntry);
  ...
  const executeDelete = () => { if (deleteTarget?.type === 'single') { deleteEntry(deleteTarget.id); toast.success("🗑️ Eintrag gelöscht"); } else if (deleteTarget?.type === 'all') { deleteAllEntries(); const emptyUser = { name: "", position: "", photo: null, workDays: [0, 510, 510, 510, 510, 270, 0] }; setUserData(emptyUser); localStorage.removeItem("kogler_last_code"); toast.success("🧹 App vollständig zurückgesetzt"); } setDeleteTarget(null); };
  ...
  const exportData = async () => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
          ...
          if (navigator.canShare && navigator.canShare({ files: [file] })) { try { await navigator.share({ files: [file], title: 'Vertel Backup', text: 'Backup meiner Stunden' }); toast.success("📤 Export erfolgreich!", { id: toastId }); return; } catch (shareError) { if (shareError.name === 'AbortError') { toast.dismiss(toastId); return; } } }
          const blob = new Blob([json], { type: "application/json" });
          const url = URL.createObjectURL(blob);
          const a = document.createElement("a"); a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
          toast.success("💾 Download gestartet!", { id: toastId });
      }
    } catch (e) { toast.error(`❌ Export Fehler: ${e.message}`, { id: toastId, duration: 5000 }); }
  };

  const handleImport = (event) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = (e) => { try { const d = JSON.parse(e.target.result); if(d.entries) importEntries(d.entries); if(d.user) setUserData(d.user); toast.success("📥 Daten erfolgreich importiert!"); } catch { toast.error("❌ Fehler: Datei ungültig."); } finally { event.target.value = ""; } }; reader.readAsText(file); };
  ...
  return (
    <div className="min-h-screen w-screen font-sans bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 overflow-x-hidden relative">
      ...
      {showOnboarding && ( <OnboardingWizard onFinish={handleOnboardingFinish} initialData={userData} /> )}
      <ConfirmModal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={executeDelete} title={deleteTarget?.type === 'all' ? "Alles löschen?" : "Eintrag löschen?"} message={deleteTarget?.type === 'all' ? "Möchtest du wirklich alle Einträge unwiderruflich löschen? Auch dein Profil wird zurückgesetzt." : "Möchtest du diesen Eintrag wirklich entfernen?"} />
      ...
      <header className="fixed top-0 left-0 right-0 bg-slate-900 text-white p-4 pb-6 shadow-xl z-50 w-full transition-all" style={{ paddingTop: "calc(env(safe-area-inset-top) + 1rem)" }}>
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            {view !== "dashboard" && view !== "report" ? ( <button onClick={() => { setView("dashboard"); setEditingEntry(null); }} className="p-2 hover:bg-slate-700 rounded-full transition-colors"><ArrowLeft size={24} /></button> ) : ( <div className="w-10 h-10 rounded-xl overflow-hidden flex items-center justify-center bg-slate-800 shadow-inner"><img src={KoglerLogo} alt="Logo" className="w-full h-full object-contain" /></div> )}
            <div> <h1 className="font-bold text-xl leading-tight tracking-tight">{getHeaderTitle()}</h1> {view === "dashboard" && <p className="text-xs text-slate-400 font-medium mt-0.5">Kogler Aufzugsbau</p>} </div>
          </div>
          {view === "dashboard" && ( <div className="flex gap-2"> <button onClick={() => setView("settings")} className="p-2.5 bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors active:scale-95"><SettingsIcon size={20} className="text-slate-300" /></button> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setView("report")} className="bg-orange-500 hover:bg-orange-600 p-2.5 rounded-xl transition-colors shadow-lg shadow-orange-900/20"><FileBarChart size={20} className="text-white" /></motion.button> </div> )}
        </div>
      </header>
        ...
        <AnimatePresence mode="wait">
          {view === "dashboard" && (
            <motion.div key="dashboard" initial="initial" animate="in" exit="out" variants={pageVariants} transition={pageTransition} className="w-full">
              <Dashboard 
                  ...
                  // HIER ÜBERGEBEN WIR DIE VOM HOOK BERECHNETEN STATS
                  stats={stats} 
                  overtime={overtime} 
          ...
          {view === "add" && (
            <motion.div key="add" initial="initial" animate="in" exit="out" variants={pageVariants} transition={pageTransition} className="w-full">
              <EntryForm onCancel={() => { setView("dashboard"); setEditingEntry(null); }} onSubmit={handleSaveEntry} entryType={entryType} setEntryType={setEntryType} code={code} setCode={setCode} pauseDuration={pauseDuration} setPauseDuration={setPauseDuration} formDate={formDate} setFormDate={setFormDate} startTime={startTime} setStartTime={setStartTime} endTime={endTime} setEndTime={setEndTime} project={project} setProject={setProject} lastWorkEntry={lastWorkEntry} existingProjects={uniqueProjects} allEntries={entries} isEditing={!!editingEntry} isLiveEntry={isLiveEntry} userData={userData} />
            </motion.div>
          )}
          {view === "settings" && (
            <motion.div key="settings" initial="initial" animate="in" exit="out" variants={pageVariants} transition={pageTransition} className="w-full">
              <Settings userData={userData} setUserData={setUserData} theme={theme} setTheme={setTheme} autoBackup={autoBackup} setAutoBackup={setAutoBackup} onExport={exportData} onImport={() => fileInputRef.current?.click()} onDeleteAll={() => setDeleteTarget({ type: 'all' })} onCheckUpdate={handleManualUpdateCheck} />
            </motion.div>
          )}
          {view === "report" && (
            <motion.div key="report" initial="initial" animate="in" exit="out" variants={reportVariants} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 z-[200] w-full h-full">
              <Suspense fallback={ <div className="flex items-center justify-center h-full w-full bg-slate-900/50 backdrop-blur-sm"> <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-xl flex items-center gap-3"> <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-orange-500"></div> <span className="font-bold text-slate-700 dark:text-white">Lade PDF-Modul...</span> </div> </div> }>
                <PrintReport entries={entriesWithHolidays} monthDate={currentDate} employeeName={userData.name} userPhoto={userData.photo} onClose={() => setView("dashboard")} onMonthChange={setCurrentDate} userData={userData} />
              </Suspense>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {!showOnboarding && (
        <>
          <AnimatePresence>
            {view === "dashboard" && (
              <motion.button initial={{ scale: 0, rotate: 90 }} animate={{ scale: 1, rotate: 0 }} exit={{ scale: 0, rotate: 90 }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); startNewEntry(); Haptics.impact({ style: ImpactStyle.Medium }).catch(() => {}); }} style={{ bottom: "calc(3.5rem + env(safe-area-inset-bottom))" }} className="fixed right-6 bg-slate-900 dark:bg-orange-500 hover:bg-slate-800 dark:hover:bg-orange-600 text-white w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-colors z-[9999] cursor-pointer touch-manipulation">
                <Plus size={28} />
              </motion.button>
            )}
          </AnimatePresence>
          {view === "dashboard" && (
             <LiveTimerOverlay timerState={timerState} onStart={handleStartLive} onStop={handleStopLive} onPause={() => { Haptics.impact({style: ImpactStyle.Light}); pauseTimer(); }} onResume={() => { Haptics.impact({style: ImpactStyle.Light}); resumeTimer(); }} targetMinutes={todayTarget} />
          )}
        </>
      )}
    </div>
