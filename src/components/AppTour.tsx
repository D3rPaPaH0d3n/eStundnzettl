import { useState, useEffect, useLayoutEffect, useCallback, useMemo, useId } from "react";
...
const AppTour = ({ onClose }: Props) => {
  ...
  const cardContainerStyle = (() => {
    if (!pointerStyle) return undefined;
    const vh = getViewportSize().height;
    const safeTop = 24;
    const safeBottom = 24;
    const pointerTop = Number(pointerStyle.top);
    const pointerHeight = Number(pointerStyle.height);
    const spaceAbove = pointerTop - CARD_GAP - safeTop;
    const spaceBelow = vh - (pointerTop + pointerHeight) - CARD_GAP - safeBottom;

    if (spaceBelow >= ESTIMATED_CARD_HEIGHT || spaceBelow >= spaceAbove) {
      return {
        top: Math.min(
          pointerTop + pointerHeight + CARD_GAP,
    ...
    return {
      bottom: Math.min(vh - pointerTop + CARD_GAP, vh - ESTIMATED_CARD_HEIGHT - safeTop),
    };
  })();

  const cardPositionClass = pointerStyle ? "" : "top-1/2 -translate-y-1/2";
  const overlayMaskId = `app-tour-spotlight-${useId().replace(/:/g, "")}`;
          ...
          <svg className="absolute inset-0 h-full w-full" aria-hidden="true">
            <defs>
              <mask id={overlayMaskId}>
                ...
                <rect
                  x={Number(pointerStyle.left)}
                  y={Number(pointerStyle.top)}
                  width={Number(pointerStyle.width)}
                  height={Number(pointerStyle.height)}
                  rx={Number(pointerStyle.height) / 2}
                  ry={Number(pointerStyle.height) / 2}
                  fill="black"
                />
              </mask>
            </defs>
