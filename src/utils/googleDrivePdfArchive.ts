/**
 * googleDrivePdfArchive.ts — Parallel GDrive-Integration fuer das automatische
...
export const AUTH_SCOPE_PDF = 'https://www.googleapis.com/auth/drive.file';
export const TOKEN_STORAGE_KEY_PDF = 'google_auth_state_pdf';
export const FOLDER_ID_STORAGE_KEY = 'google_pdf_archive_folder_id';
const ARCHIVE_FOLDER_NAME = 'eStundnzettl Archiv';
const SESSION_TOKEN_MAX_AGE_MS = 45 * 60 * 1000;

// Interface für GoogleDriveBackupPlugin (entspricht der Definition in globals.d.ts)
interface GoogleDriveBackupPluginInterface {
...
const GoogleDriveBackupPlugin = registerPlugin('GoogleDriveBackup') as GoogleDriveBackupPluginInterface;
...
export const getStoredAuthPdf = (): AuthState | null => {
  try {
...
const saveStoredAuthPdf = (auth: Partial<AuthState> & { accountEmail?: string | null }): void => {
...
const parseNativeError = (error: any, fallback = 'GOOGLE_DRIVE_PDF_ERROR'): string => {
 ...
 */
export async function connectGoogleDrivePdf(): Promise<ConnectResult> {
 ...
 */
export async function getGoogleDrivePdfStatus(): Promise<StatusResult> {
  const stored = getStoredAuthPdf();
  if (!(await nativeAvailable())) {
    return {
      available: false,
...
async function findOrCreateArchiveFolder(): Promise<string> {
  ...
  if (cached) {
    ...
    try {
      ...
      if (verifyRes.ok) {
        const data = await verifyRes.json() as DriveFile;
  ...
  if (searchRes.ok) {
    const data = await searchRes.json() as DriveFileList;
 ...
 *
 * @param filename  z.B. 'Stundenzettel_2026-04_Max.pdf'
 * @param base64    base64-kodierter PDF-Inhalt (ohne data:-Prefix)
 * @param _blob     Ignoriert — bleibt fuer API-Kompatibilitaet
 */
export async function uploadPdfArchiveFile(filename: string, base64: string, _blob?: Blob): Promise<any> {
...
// Die Konstanten und getStoredAuthPdf sind bereits oben exportiert
