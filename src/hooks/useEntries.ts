...
export function useEntries() {
  ...
  const addEntry = useCallback(async (entry: TimeEntry) => {
    ...
    try {
      await insertEntry(timeEntryToInput(entry));
    } catch (err) {
  ...
  const updateEntry = useCallback(async (updatedEntry: TimeEntry) => {
    ...
    try {
      await updateEntryInDb(timeEntryToInput(updatedEntry));
    } catch (err) {
      logger.error("[useEntries] SQLite-Write fehlgeschlagen:", err);
      if (previous) {
  ...
  const deleteEntry = useCallback(async (id: number) => {
    let removed: TimeEntry | null = null;
    setEntries((prev) => {
  ...
  const importEntries = useCallback(async (newEntries: TimeEntry[]) => {
    const backup: TimeEntry[] = [];
    setEntries((prev) => { backup.push(...prev); return newEntries; });
    try {
      const entriesInput = newEntries.map(timeEntryToInput);
      await bulkInsertEntries(entriesInput);
    } catch (err) {
      logger.error("[useEntries] SQLite-Write fehlgeschlagen:", err);
      setEntries(backup);
